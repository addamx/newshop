...
<body>
<form action="/newshop/index.php/admin/goods/edit?id=6" method="post" id="aform" enctype="multipart/form-data">
	<P><label for="name">商品名称:  </label><input type="text" id='name' name="name" value="<?php echo ($info["name"]); ?>"/></P>
	<P><label for="sn">商品货号:  </label><input type="text" id='sn' name="sn" value="<?php echo ($info["sn"]); ?>" /></P>
	...
	<p><label for="ori_img">重新上传图片: </label><input type="file" id="ori_img" name="ori_img" onchange="showimg();"/></p>
	<div><img id='img_show' src="<?php echo (UPLOAD_URL); ?>/<?php echo ($info["goods_img"]); ?>" height="200px"><span></span></div>
